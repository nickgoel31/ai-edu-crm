...
export default function ModuleErrorBoundary({
  ...
  useEffect(() => {
    console.error("Agent Conversation Log Error Boundary caught error:", error);
  }, [error]);
  ...
  return (
    <div className="max-w-lg mx-auto my-12 p-8 bg-card border border-border rounded-xl shadow-sm text-center">
      ...
      <h2 className="text-lg font-bold text-zinc-100">
        Something went wrong in Agent Profile
      </h2>
      <p className="text-xs text-zinc-400 mt-2 mb-6">
        {error.message || "An unexpected error occurred while loading agent details."}
      </p>
