...
export default function StudentsDuplicatesPage() {
  ...
  const fetchMatches = async () => {
    setIsLoading(true);
    try {
      const res = await fetch(`/api/duplicates?entityType=STUDENT&status=${activeTab}`);
      ...
      console.error("Failed to load duplicate student matches:", err);
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-border/60 pb-5">
        <div className="space-y-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight text-white font-heading flex items-center gap-2.5">
            ...
            <span>Student Deduplication Queue</span>
          </h1>
          <p className="text-xs text-zinc-400">
            Review matching student profiles, compare enrollments & documents, and merge duplicate records.
          </p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
        ...
        <div className="relative w-full sm:w-64">
          ...
          <Input
            ...
            placeholder="Search student duplicates..."
      ...
      {isLoading ? (
        ...
        <div className="py-16 text-center space-y-3 bg-[#171717] border border-[#262626] rounded-xl">
          ...
          <h3 className="text-sm font-semibold text-white">
            {activeTab === "PENDING"
              ? "All Clean! No Pending Duplicate Students"
              : activeTab === "MERGED"
              ? "No Merged Student Records Found"
              : "No Dismissed Matches Found"}
          </h3>
          <p className="text-xs text-zinc-400 max-w-sm mx-auto">
            {activeTab === "PENDING"
              ? "When new student enrollments share contact information with existing student profiles, they will appear here for side-by-side review."
